...
use crate::state::{AffiliateAccount, AffiliateTier, AgentStatus, ProtocolConfig};
use crate::utils::{
    calculate_affiliate_commission, calculate_referrer_commission, calculate_tier,
    get_tier_commission_bps,
};
...
pub fn create_agent(
    ...
    let mut fee_amount = ctx
        .accounts
        .protocol_config
        .calculate_base_fee("create_agent");
    ...
    if let Some(collection) = &collection_mint {
        ...
        if *collection == ctx.accounts.protocol_config.genesis_collection_mint {
            ...
            let (expected_partner_pda, _) =
                Pubkey::find_program_address(&[b"partner", collection.as_ref()], ctx.program_id);
    ...
    if fee_amount > 0 && affiliate_pubkey.is_some() {
        ...
        let (tier, referrer_pubkey, needs_init) =
            if let Some(aff_acc_info) = &ctx.accounts.affiliate_account {
        ...
        let commission_bps = if !needs_init && ctx.accounts.affiliate_account.is_some() {
            ...
            match AffiliateAccount::try_deserialize(&mut &data[..]) {
                Ok(aff_acc) => {
                    aff_acc.get_commission_bps(ctx.accounts.protocol_config.max_affiliate_bps)
                }
                Err(_) => {
        ...
        if needs_init && ctx.accounts.affiliate_account.is_some() {
            ...
            aff_acc.total_sales = aff_acc
                .total_sales
                .checked_add(1)
                .ok_or(MainframeError::CounterOverflow)?;

            aff_acc.total_revenue = aff_acc
                .total_revenue
        ...
        if referrer_commission > 0 && ctx.accounts.referrer.is_some() {
            ...
            if let Some(ref_acc_info) = &ctx.accounts.referrer_account {
                if !ref_acc_info.data_is_empty() && ref_acc_info.lamports() > 0 {
                    ...
                    ref_acc.referree_sales = ref_acc
                        .referree_sales
                        .checked_add(1)
                        .ok_or(MainframeError::CounterOverflow)?;

                    ref_acc.referree_revenue = ref_acc
                        .referree_revenue

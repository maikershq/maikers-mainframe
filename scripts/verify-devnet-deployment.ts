...
const PROGRAM_ID = new PublicKey(
  "8MLkFLHS2AfTEAzewBcb4qbVjNUkUc4843VASZmn1VZb"
);
...
async function verifyDevnetDeployment() {
  ...
  if (!fs.existsSync(authorityPath)) {
    throw new Error(
      "Authority keypair not found. Run init:config:devnet first."
    );
  }
  ...
  try {
    ...
    console.log(
      `   Genesis Collection: ${config.genesisCollectionMint.toBase58()}`
    );
    console.log(`   Protocol Treasury: ${config.protocolTreasury.toBase58()}`);
    console.log(
      `   Validator Treasury: ${config.validatorTreasury.toBase58()}`
    );
    console.log(`   Network Treasury: ${config.networkTreasury.toBase58()}`);
    console.log(`   Protocol Paused: ${config.paused}`);
    console.log(`\n   Fee Structure:`);
    console.log(
      `   - Create Agent: ${config.fees.createAgent.toNumber() / 1e9} SOL`
    );
    console.log(
      `   - Update Config: ${
        config.fees.updateAgentConfig.toNumber() / 1e9
      } SOL`
    );
    console.log(
      `   - Transfer Agent: ${config.fees.transferAgent.toNumber() / 1e9} SOL`
    );
    console.log(
      `   - Pause Agent: ${config.fees.pauseAgent.toNumber() / 1e9} SOL`
    );
    console.log(
      `   - Close Agent: ${config.fees.closeAgent.toNumber() / 1e9} SOL`
    );
    console.log(
      `   - Execute Action: ${config.fees.executeAction.toNumber() / 1e9} SOL`
    );
    ...
    console.log(`\n   Protocol Limits:`);
    console.log(
      `   - Max Partner Collections: ${config.maxPartnerCollections}`
    );
    console.log(`   - Max Affiliate BPS: ${config.maxAffiliateBps / 100}%`);

    // Check treasuries have proper balances
    console.log(`\n💰 Checking treasury balances...`);
    const protocolBalance = await connection.getBalance(
      config.protocolTreasury
    );
    const validatorBalance = await connection.getBalance(
    ...
    if (error.message?.includes("Account does not exist")) {
      throw new Error(
        "❌ Protocol config not initialized. Run 'yarn init:config:devnet' first."
      );
    }
    throw error;
  }

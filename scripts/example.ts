...
async function initializeMainnet() {
  ...
  await initializeConfig({
    ...
    fees: {
      ...
      updateAgentConfig: 5000000, // 0.005 SOL - Standard mainnet fee
...
async function initializeWithCustomAuthority() {
  ...
  await initializeConfig({
    ...
    fees: {
      ...
      updateAgentConfig: 3750000, // 0.00375 SOL - Custom configuration updates
...
async function main() {
  ...
  switch (example) {
    ...
    default:
      console.log(
        "Usage: EXAMPLE=devnet|mainnet|custom ts-node scripts/example.ts"
      );
      break;
  }
}
